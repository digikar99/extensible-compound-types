...
(defmacro excl:defun (name lambda-list &body body &environment env)
  `(cl:defun ,name ,@(rest (macroexpand-1 `(excl:lambda ,lambda-list ,@body) env))))

(defmacro excl:defmacro (name lambda-list &body body &environment env)
  `(cl:defmacro ,@(rest (macroexpand-1 `(excl:defun ,name ,lambda-list ,@body) env))))

(defmacro excl:define-compiler-macro (name lambda-list &body body &environment env)
  `(cl:define-compiler-macro ,@(rest (macroexpand-1 `(excl:defun ,name ,lambda-list ,@body) env))))

(defmacro excl:defmethod (name &rest args &environment env)
  (if (listp (first args))
      `(cl:defmethod ,@(rest (macroexpand-1 `(excl:defun ,name ,@args) env)))
      `(cl:defmethod ,name ,(first args)
...
(defmacro excl:define-condition (name (&rest parent-types) (&rest slot-specs)
                                 &body options
                                 &environment env)
  `(cl:define-condition ,name ,parent-types
     ,slot-specs
     ,@(let ((report-value (second (assoc :report options))))
         (if (and (listp report-value)
                  (member (first report-value) '(cl:lambda excl:lambda)))
...
(defmacro excl:do (varlist endlist &body body &environment env)
  `(cl:do ,@(rest (macroexpand-1 `(excl:destructuring-bind ,varlist ,endlist ,@body) env))))

(defmacro excl:do* (varlist endlist &body body &environment env)
  `(cl:do* ,@(rest (macroexpand-1 `(excl:do ,varlist ,endlist ,@body) env))))

(defmacro excl:do-all-symbols ((var &optional (result-form nil result-form-p))
                               &body body-decls &environment env)
  `(cl:do-all-symbols ,(if result-form-p
                           `(,var ,result-form)
                           `(,var))
...
(defmacro excl:do-external-symbols ((var
                                    ...
                                    &body body-decls &environment env)
  `(cl:do-external-symbols ,(cond ((and packagep result-form-p)
